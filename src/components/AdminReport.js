...
//     try {
//       setLoading(true);
//       const res = await axios.get('https://database-production-3a68.up.railway.app/api/reports/driver-slots');
//       setData(res.data || []);
//     } catch (error) {
...
const AdminReport = () => {
  ...
  const handleUpdateSlot = async (slotId) => {
    try {
      await axios.put(`https://database-production-3a68.up.railway.app/api/slots/update/${slotId}`, updatedSlot);
      setEditingSlot(null);
      fetchData();
    } catch (err) {
  ...
  const handleDeleteSlot = async (slotId) => {
    try {
      ...
      await axios.delete(`https://database-production-3a68.up.railway.app/api/slots/delete/${slotId}`);
      fetchData();
    } catch (err) {
